...
//   const menProducts = data.products.filter((ele) => ele.Category === "Men");
...
const Men = () => {
  ...
  const LTHSort = () => {
    dispatch(LTHProducts(searchItem));
  };
      ...
      <div className="bg-white">
        <div className="w-11/12 mx-auto py-3">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <h1 className="text-xl md:text-2xl mb-2 md:mb-0">Men Collection</h1>
              ...
              .map((product) => (
                <Link to={`/product/${product.id}`} key={product.id}>
                  <div className="cursor-pointer m-4" key={product.id}>
                    <img
                      className="h-60 md:h-72 object-fill rounded-lg w-full hover:scale-105 transition-transform duration-300 ease-in-out"
                      src={product.Image}
                      alt={product.Title}
                    />
